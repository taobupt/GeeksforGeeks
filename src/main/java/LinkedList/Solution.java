...
public class Solution {
   ...
   public ListNode rotate(ListNode head,int k){
    ...
    //ugly way
    public TreeNode binary2list(TreeNode root){
        if(root==null||(root.left==null && root.right==null))
            return root;
        TreeNode left=binary2list(root.left);
        TreeNode right=binary2list(root.right);
        TreeNode node=left;
    ...
    //this is inorder way
    //so you can image something like the inorder traversal
    ...
    public TreeNode binary2listOptimal(TreeNode root){
